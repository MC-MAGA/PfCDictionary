...
% process times breakdown of sessions into trials and epochs

n = numel(learning_Sessions);
SessionElements = BreakdownSessionTimes(datapath,learning_Sessions);

load([filepath 'SessionReward']);  % reward curve data per learning session
...
%% inset: separate histogram of all rest period intervals - don't need if not analysing
% figure('Units', Units, 'PaperPositionMode', 'auto','Position',[10 15 1.5 2.5]); hold on;
% scatter(ones(n,1),[session(:).restT],'MarkerEdgeColor',[0.7 0.2 0.7],'SizeData',5); hold on
% line([0.9 1.1],[mean([session(:).restT]),mean([session(:).restT])],'Color','k','Linewidth',errorlinewidth)
% line([1 1],[mean([session(:).restT])-2*std([session(:).restT])/sqrt(n),mean([session(:).restT])+2*std([session(:).restT])/sqrt(n)],'Color','k','Linewidth',errorlinewidth)
% set(gca,'XTick',[],'YTick',[0,250,500,750,1000]);
% pos = get(gca,'Position');
% set(gca,'Position',[pos(1) pos(2) 0.25 pos(4)])
% ylabel('Post-task rest (s)','FontSize',fontsize); 
% set(gca,'FontName','Helvetica','FontSize',fontsize);
% set(gca,'Box','off','TickDir','out','LineWidth',axlinewidth);
% 
% print('-depsc',['Fig1_inset_rest_duration']);
...
% figure('Units', Units, 'PaperPositionMode', 'auto','Position',[10 15 4 4]); hold on;
% line([0 0],[-15 20],'Color',[0.7 0.7 0.7])
% for iS = 1:nSessions
%     x = [1:session(iS).nTrials] - session(iS).learningtrial;
%     y = session(iS).cumR - session(iS).cumR(session(iS).learningtrial);
%     stairs(x,y)
% end
% axis tight
...
% exportfig(gcf,[exportpath 'Heatmap_Cumulative_Reward'],'Color',color,'Format',format,'Resolution',dpi)

% figure('Units', Units, 'PaperPositionMode', 'auto','Position',[10 15 4 4]); hold on;
% imagesc(-maxPre:maxPost-1,1:nSessions,matD_Rcumul)
% colormap(Rcmap)
% axis tight
% line([0 0],[0.5 10.5],'Color',[0.7 0.7 0.7],'LineWidth',plotlinewidth)
...
load([strategypath 'SimpleStrategy_0.9.mat'],'Sessions','pars'); % load Stable sessions - names and time-series
