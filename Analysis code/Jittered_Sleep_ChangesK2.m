...
NewPWord = emptyStruct({'Jitter'},[Nsessions,1]);
...
% for iS = 1:Nsessions    
    ...
    for iJ = 1:Njitters
        ...
        for iSh = 1:Nshuffles
            % iSh
            [NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.P,NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.ID] = RenormalisedPword(PWord(iS).Jitter(iJ).Shuffle(iSh).Pre.WordSet,PWord(iS).Jitter(iJ).Shuffle(iSh).Pre.P,PWord(iS).Jitter(iJ).Shuffle(iSh).Pre.binaryIDs,threshK);
            [NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.P,NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.ID] = RenormalisedPword(PWord(iS).Jitter(iJ).Shuffle(iSh).Post.WordSet,PWord(iS).Jitter(iJ).Shuffle(iSh).Post.P,PWord(iS).Jitter(iJ).Shuffle(iSh).Post.binaryIDs,threshK);

            % compute distance between Pre and Post session sleep in the Data
            DeltaSleep(iS).Jitter(iJ).Shuffle(iSh).D_Pre_Post = full(Hellinger(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.P,NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.ID,...
                                                    NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.P,NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.ID));
            nPre = numel(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.P);     % number of unique words
            nPost = numel(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.P);

            % joint P(word) distribution
            NPreWords = round(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.P * CountPre(iS).Jitter(iJ).Shuffle(iSh).K2);         % frequency of each word
            NPostWords = round(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.P * CountPost(iS).Jitter(iJ).Shuffle(iSh).K2);      % frequency of each word in Post
            nPreTs = CountPre(iS).Jitter(iJ).Shuffle(iSh).K2;   % number of words in Pre time-series
            nPostTs = CountPost(iS).Jitter(iJ).Shuffle(iSh).K2;
            
            DeltaSleep(iS).Jitter(iJ).Shuffle(iSh).JointIDs = union(full(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.ID),full(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.ID));
            ...
            for iW = 1:nJoint         
                ...
                npre = NPreWords(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.ID == DeltaSleep(iS).Jitter(iJ).Shuffle(iSh).JointIDs(iW)); if isempty(npre) npre = 0; end
                npost = NPostWords(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Post.ID == DeltaSleep(iS).Jitter(iJ).Shuffle(iSh).JointIDs(iW)); if isempty(npost) npost = 0; end
            ...
            for iBoot = 1:nBoot
                ...
                X = discreteinvrnd(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.P,nPreTs,1); % random sample of same number of words: IDs into ID list      
                % compute P(word)* from new samples
                Pnew = histcounts(X,nPre,'Normalization','probability');
                % compute D(Pre|Pre*)  
                DeltaSleep(iS).Jitter(iJ).Shuffle(iSh).Boot.D_Pre_PreSamp(iBoot) = full(Hellinger(NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.P,NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.ID,...
                                                                        Pnew,NewPWord(iS).Jitter(iJ).Shuffle(iSh).Pre.ID));
                % permutation test   
...
save(['DeltaSleep_K2_Jittered_N' num2str(N) '_' type '_binsize_' num2str(binsize)], 'DeltaSleep');
